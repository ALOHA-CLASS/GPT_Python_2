...
class TranscriptionWorker(QObject):
    ...
    def transcribe_chunk(self, file_path):
        try:
            ...
            # TranscriptionInfo 객체의 올바른 속성 사용
            confidence_threshold = 0.4
            
            # info 객체에서 언어 확률 또는 기타 신뢰도 정보 가져오기
            try:
                if hasattr(info, 'language_probability'):
                    confidence = float(info.language_probability)
                elif hasattr(info, 'language_confidence'):
                    confidence = float(info.language_confidence)
                else:
                    # 속성이 없으면 기본값 사용
                    confidence = 0.8  # 기본 신뢰도
                    
                print(f"언어: {info.language}, 신뢰도: {confidence}")
                
            except Exception as attr_error:
                print(f"신뢰도 정보 가져오기 오류: {attr_error}")
                confidence = 0.8  # 기본 신뢰도
...
class VoiceRecognitionApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # 음성 인식 관련 설정 (GPU 우선, 실패시 CPU)
        self.is_running = False
        self.is_paused = False  # 일시정지 상태를 위한 변수 추가
        
        # GPU 사용 시도, 실패하면 CPU 사용
        try:
            print("Whisper 모델 로드 중... (GPU 가속 시도)")
            self.model = WhisperModel(
                "small",  # GPU 사용시 더 큰 모델 가능
                device="cuda", 
                compute_type="float16",  # GPU 최적화 타입
            )
            print("Whisper 모델 로드 완료 (GPU 모드)")
        except Exception as e:
            ...
            try:
                self.model = WhisperModel(
                    "tiny",  # CPU용 가벼운 모델
                    device="cpu", 
                    compute_type="int8",  # CPU에 최적화된 타입
                    cpu_threads=4,
                    num_workers=1
                )
                print("Whisper 모델 로드 완료 (CPU 모드)")
            except Exception as cpu_e:
                print(f"CPU 모델 로드 오류: {cpu_e}, 기본 설정으로 재시도...")
